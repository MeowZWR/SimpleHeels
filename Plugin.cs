...
public unsafe class Plugin : IDalamudPlugin {
    ...
    public void SetupLivePose(bool announce = false) {
        ...
        if (livePose != null) {
            ...
            if (announce) PluginService.ChatGui.Print("正在卸载LivePose模块。", Name);
            ...
            try {
                if (Config.LivePoseEnabled) {
                    if (PluginService.PluginInterface.InstalledPlugins.Any(p => p.InternalName == "LivePose" && p.IsLoaded)) {
                        PluginService.ChatGui.PrintError("拒绝加载LivePose模块。请卸载LivePose插件。",  Name);
                        return;
                    }
                    ...
                    if (announce) PluginService.ChatGui.Print("正在加载LivePose模块。", Name);
    ...
    private void OnCommand(string command, string args) {
        ...
        if (splitArgs.Count > 0) {
            switch (splitArgs[0]
                ...
                case "livepose":
                    if (livePose == null) {
                        PluginService.ChatGui.PrintError("LivePose未启用。", Name);
                        return;
                    }
