...
public class ConfigWindow : Window {
        ...
        foreach (var (worldId, characters) in config.WorldCharacterDictionary.ToArray()) {
            ...
            foreach (var (name, characterConfig) in characters.ToArray()) {
                ...
                if (ImGui.BeginPopupContextItem(id)) {
                    if (ImGui.Selectable($"从设置中移除“{name} @ {world.Value.Name.ExtractText()}”")) {
            ...
            if (selectedCharacter != null) {
                ...
                if (ImGui.CollapsingHeader("名称匹配")) {
                    ...
                    foreach (var c in selectedGroup.Characters.ToArray()) {
                        ...
                        ImGui.SameLine();
                        ImGui.SetNextItemWidth(ImGuiHelpers.GlobalScale * 140);
                        if (ImGui.BeginCombo("##world", c.World == ushort.MaxValue ? "非玩家" : PluginService.Data.GetExcelSheet<World>()?.GetRowOrDefault(c.World)?.Name.ExtractText() ?? $"世界#{c.World}", ImGuiComboFlags.HeightLargest)) {
                ...
                if (config.UseModelOffsets) {
                    ...
                    ImGui.SameLine();
                    ImGui.Checkbox("启用模型偏移的“复制属性”按钮。", ref config.CopyAttributeButton);
                }
                ...
                using (ImRaii.PushId("TempOffsets")) {
                    ...
                    ImGui.Checkbox("将窗口限制在《最终幻想14》窗口内", ref config.TempOffsetWindowLockInViewport);
                ...
                using (ImRaii.PushId("LivePoseConfig")) {
                    ...
                    if (ImGui.Checkbox("启用LivePose", ref config.LivePoseEnabled)) {
    ...
    private unsafe void DrawCharacterView(CharacterConfig? characterConfig) {
        ...
        if (characterConfig is GroupConfig gc) {
            ...
            if (target is ICharacter) {
                ...
                if (target is IPlayerCharacter pc) {
                    ImGui.TextDisabled($"预览显示基于 {target.Name.TextValue} ({pc.HomeWorld.Value.Name.ExtractText()})");
                }
                else {
        ...
        if (characterConfig is not IpcCharacterConfig && config.ShowCopyUi && newWorld != 0)
        {
            ...
            if (newWorld == 0)
            {
                newWorld = homeWorldId;
            }
                        
            var worldName = (newWorld == homeWorldId || newWorld == 0)
                ? PluginService.Data.GetExcelSheet<World>()!.GetRow(homeWorldId).Name.ToDalamudString().TextValue
                : PluginService.Data.GetExcelSheet<World>()!.GetRow(newWorld)!.Name.ToDalamudString().TextValue;

            if (ImGui.BeginCombo("世界", worldName))
            {
                var lastDc = string.Empty;

                var worlds = PluginService.Data.GetExcelSheet<World>()!
                    .Where(w => w.IsPlayerWorld())
                    .OrderBy(w => w.RowId == homeWorldId ? 0 : 1)
                    .ThenBy(w => w.DataCenter.Value.Name.ExtractText() ?? string.Empty)
                ...
                foreach (var world in worlds)
                {
                    ...
                    string worldDisplayText = $"{world.Name.ToDalamudString().TextValue} (ID: {world.RowId})";

                    if (ImGui.Selectable($"  {worldDisplayText}", world.RowId == newWorld))
                    {
        ...
        using (ImRaii.Disabled(characterConfig is IpcCharacterConfig)) {
                ...
                if (ImGui.BeginTable("OffsetsTable", 5, ImGuiTableFlags.Resizable)) {
                    ...
                    for (var i = 0; i < characterConfig.HeelsConfig.Count; i++) {
                        ...
                        if ((heelConfig.Slot == ModelSlot.Feet && ((heelConfig.PathMode == false && activeFootwear == heelConfig.ModelId) || (heelConfig.PathMode && activeFootwearPath != null && activeFootwearPath.Equals(heelConfig.Path, StringComparison.OrdinalIgnoreCase)))) || (heelConfig.Slot == ModelSlot.Legs && ((heelConfig.PathMode == false && activeLegs == heelConfig.ModelId) || (heelConfig.PathMode && activeLegsPath != null && activeLegsPath.Equals(heelConfig.Path, StringComparison.OrdinalIgnoreCase)))) || (heelConfig.Slot == ModelSlot.Top && ((heelConfig.PathMode == false && activeTop == heelConfig.ModelId) || (heelConfig.PathMode && activeTopPath != null && activeTopPath.Equals(heelConfig.Path, StringComparison.OrdinalIgnoreCase))))) {
                            ShowActiveOffsetMarker(activeCharacterAsCharacter != null, heelConfig.Enabled, activeHeelConfig?.Is(heelConfig) ?? false, "当前穿戴");
                            if (heelConfig.Enabled) {
                                wearingMatchCount++;
                                usingDefault = false;
                            }
            ...
            if (emoteOffsetsOpen && characterConfig.EmoteConfigs != null) {
                ...
                if (ImGui.BeginTable("emoteOffsets", (characterConfig is IpcCharacterConfig ? 7 : 9) - (space < 1000 ? 3 : 0), ImGuiTableFlags.NoClip | (space < 1000 ? ImGuiTableFlags.None : ImGuiTableFlags.Resizable))) {
                    ...
                    if (characterConfig is not IpcCharacterConfig) {
                        ImGui.TableSetupColumn("启用", ImGuiTableColumnFlags.WidthFixed, checkboxSize * 4 + 3 * ImGuiHelpers.GlobalScale);
                        ImGui.TableSetupColumn("标签", ImGuiTableColumnFlags.WidthStretch, 0.15f);
                    }
                    ImGui.TableSetupColumn("情感动作", ImGuiTableColumnFlags.WidthStretch, 0.3f);
                    ImGui.TableSetupColumn("相对", ImGuiTableColumnFlags.WidthFixed, checkboxSize);
                    if (space >= 1000) {
                        ImGui.TableSetupColumn("高度偏移", ImGuiTableColumnFlags.WidthStretch, 0.1625f);
                        ImGui.TableSetupColumn("前后偏移", ImGuiTableColumnFlags.WidthStretch, 0.1625f);
                        ImGui.TableSetupColumn("左右偏移", ImGuiTableColumnFlags.WidthStretch, 0.1625f);
                        ImGui.TableSetupColumn("    旋转", ImGuiTableColumnFlags.WidthStretch, 0.1625f);
                    } else {
                        ImGui.TableSetupColumn("偏移", ImGuiTableColumnFlags.WidthStretch, 0.65f);
                    }
                    ...
                    for (var emoteIndex = 0; emoteIndex < characterConfig.EmoteConfigs.Count; emoteIndex++) {
                        ...
                        if (characterConfig is not IpcCharacterConfig) {
                            ...
                            using (ImRaii.PushStyle(ImGuiStyleVar.ItemSpacing, new Vector2(1))) {
                                ...
                                if (e.Locked == false && !ImGui.GetIO().KeyShift && ImGui.IsItemHovered(ImGuiHoveredFlags.AllowWhenDisabled)) {
                                    ImGui.SetTooltip("按住SHIFT重置所有值为零。");
                                }
                        ...
                        if (space >= 1000) {
                            ...
                            using (ImRaii.ItemWidth(ImGui.GetContentRegionAvail().X - ImGui.CalcTextSize("  旋转  ").X)) {
                                ImGuiExt.FloatEditor("高度", ref e.Offset.Y, 0.0001f, allowPlusMinus: characterConfig is not IpcCharacterConfig, resetValue: 0f);
                                ImGuiExt.FloatEditor("前后", ref e.Offset.Z, 0.0001f, allowPlusMinus: characterConfig is not IpcCharacterConfig, resetValue: 0f);
                                ImGuiExt.FloatEditor("两侧", ref e.Offset.X, 0.0001f, allowPlusMinus: characterConfig is not IpcCharacterConfig, resetValue: 0f);
                                var rot = e.Rotation * 180f / MathF.PI;
                                if (ImGuiExt.FloatEditor("旋转", ref rot, format: "%.0f", allowPlusMinus: characterConfig is not IpcCharacterConfig, customPlusMinus: 1, resetValue: 0f)) {
                                    if (rot < 0) rot += 360;
                                    if (rot >= 360) rot -= 360;
                                    e.Rotation = rot * MathF.PI / 180f;
                                }
                        ...
                        ShowActiveOffsetMarker(activeCharacterAsCharacter != null && activeEmote != null && (activeEmote == e.Emote || (characterConfig is not IpcCharacterConfig && e.Editing == false && e.LinkedEmotes.Contains(activeEmote))), e.Enabled, activeHeelConfig?.Is(e) ?? false, "当前正在执行情感动作。");
                        ...
                        if (characterConfig is IpcCharacterConfig || e.Editing) {
                            ...
                            foreach (var linked in e.LinkedEmotes.ToArray()) {
                                ...
                                if (fl) {
                                    fl = false;
                                    ImGui.GetWindowDrawList().AddText(ImGui.GetFont(), ImGui.GetFont().FontSize, ImGui.GetItemRectMin() + new Vector2(ImGui.GetContentRegionAvail().X + ImGui.GetItemRectSize().X, 0), ImGui.GetColorU32(ImGuiCol.TextDisabled), "链接的情感动作使用与其基础相同的偏移量。");
                                }
                                ...
                                ShowActiveOffsetMarker(activeCharacterAsCharacter != null && activeEmote == linked, e.Enabled, activeHeelConfig?.Is(e) ?? false, "情感动作当前正在执行");
                            }
            ...
            ImGuiExt.Separator();
            ImGuiExt.FloatEditor("默认偏移", ref characterConfig.DefaultOffset, 0.001f, allowPlusMinus: characterConfig is not IpcCharacterConfig, resetValue: 0f);
            ImGui.SameLine();
